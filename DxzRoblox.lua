-- === BẮT ĐẦU: HỆ THỐNG KEY ===
local CORRECT_KEY = "DXZDEPTROAI" -- <-- thay key bạn muốn ở đây

local Player = game:GetService("Players").LocalPlayer
-- Tạo GUI đơn giản để nhập key (chỉ local, không ảnh hưởng server)
local screenGui = Instance.new("ScreenGui")
screenGui.ResetOnSpawn = false
screenGui.Name = "DxzKeyGui"
...
title.Text = "Dán key admin"
title.TextScaled = true
title.TextColor3 = Color3.fromRGB(255,255,255)

local txtBox = Instance.new("TextBox", frame)
txtBox.Size = UDim2.new(1, -24, 0, 48)
txtBox.Position = UDim2.new(0, 12, 0, 44)
txtBox.PlaceholderText = "Nhập key ở đây"
txtBox.Text = ""
txtBox.ClearTextOnFocus = false
txtBox.TextScaled = true
txtBox.BackgroundColor3 = Color3.fromRGB(35,35,35)
txtBox.TextColor3 = Color3.fromRGB(255,255,255)
txtBox.BorderSizePixel = 0

local btn = Instance.new("TextButton", frame)
btn.Size = UDim2.new(1, -24, 0, 40)
btn.Position = UDim2.new(0, 12, 0, 102)
btn.Text = "Xác nhận"
btn.TextScaled = true
...
-- Hàm xử lý khi key đúng -> load phần còn lại của script
local function LoadMainScript()
    -- Xóa GUI
    screenGui:Destroy()

    -- --- DÒNG ĐÃ BỊ THAY THẾ --- 
    -- Đây là chỗ load script Fluent như trước
    local v14 = loadstring(game:HttpGet("https://github.com/dawid-scripts/Fluent/releases/latest/download/main.lua"))();

    -- Tiếp tục phần script gốc (phần bạn đã gửi) sẽ dùng v14, v15, ...
    -- (Phần còn lại của file của bạn không cần thay đổi)
end

-- Nút bấm kiểm tra key
btn.MouseButton1Click:Connect(function()
    if txtBox.Text == CORRECT_KEY then
        btn.Text = "✅ Key hợp lệ"
        LoadMainScript()
    else
        btn.Text = "❌ Sai key"
...
-- Nếu muốn cho phép bấm Enter trong TextBox:
...
local v15 = v14:CreateWindow({
    Title = "Dxz ROBLOX [ Bản ADMIN ]",-- 
tên bạn muốn đặt
    SubTitle = "Bởi Dxz",-- tên bạn
    TabWidth = 160,
    Theme = "Dark",--màu
    Acrylic = false,
    ...
    function getHighestPoint()
        ...
        for v728, v729 in pairs(game:GetService("Workspace").Map.MysticIsland:GetDescendants()) do
            if v729:IsA("MeshPart") then
                if (v729.MeshId == "rbxassetid://138538611015843") then --thay id ảnh vào
                    return v729;
                end
            end
        end
